...
import { ValidateAtualizacaoCurso } from '../../schemas/curso';
...
export const atualizarCursoHandler = new Handler(
  async (context): Promise<NavigationResult<null>> => {
    ...
    if (!ValidateAtualizacaoCurso(body)) {
      return {
        status: 400,
        body: {
    ...
    const service: DatabaseService<NavigationResult<null>> = async (
      ...
      const result = await RepositorioProjeto.atualizarCurso(
        idProjeto,
        idCurso,
        body,
        db,
