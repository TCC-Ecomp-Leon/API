...
exports.atualizarCursoHandler = void 0;
...
exports.atualizarCursoHandler = new handler_1.default(async (context) => {
    ...
    if (!curso_1.ValidateAtualizacaoCurso(body)) {
        return {
            status: 400,
            body: {
    ...
    const service = async (db, session) => {
        ...
        const result = await RepositorioProjeto_1.default.atualizarCurso(idProjeto, idCurso, body, db, session);
        if (!result.success) {
            throw result.error;
        }
        return {
